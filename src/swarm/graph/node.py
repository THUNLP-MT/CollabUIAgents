...
class Node(ABC):
    ...
    async def execute(self, **kwargs):
            ...
            if self.combine_inputs_as_one:
                ...
                for predecessor in self.predecessors:
                    ...
                    if predecessor_outputs is not None and isinstance(predecessor_outputs, list):
                        text = predecessor_outputs[0]['output']
                        text_comp = ''.join(text)
                        combined_inputs.append(text_comp)
                        if not self.inputs:
                            node_input = ''.join(combined_inputs) + '\nYour answer must in this format:\nReason: ...\nAction: {{"action_type":...}}\n\n Your Answer:\n'
                        else:
                            pre_agent_output = ''
                            for idx, str_text in enumerate(combined_inputs):
                                pre_agent_output = pre_agent_output + 'Agent {}:'.format(idx) + str_text + '\n\n'
                            node_input = ''.join(self.inputs) + 'Here are some actions from other UI agents (For reference only, their reasoning may contain errors). You should reflect on their answer (Determine whether it is correct) and then provide your own answer: \n {}\n'.format(pre_agent_output) + '\nYour answer must in this format:\nReason: ...\nAction: {{"action_type":...}}\n\n Your Answer:\n'
                tasks.append(asyncio.create_task(self._execute(node_input, **kwargs)))
            else: 
                for predecessor in self.predecessors:
                    predecessor_outputs = predecessor.outputs
        ...
        elif not self.predecessors: 
            tasks = [asyncio.create_task(self._execute(''.join(self.inputs), **kwargs))]
        else:

            ...
            for root1, dirs1, files1 in os.walk(dir_path):
                for file in tqdm(files1):
                    if file.endswith('.pkl.gz'):
                        ...
                        if is_successful != 1.0 and is_successful != 0.0:
                            nan_num += 1
                        if is_successful == 1.0 or is_successful == 0.0:
                            ...
                            summary_UI = episode_data['summary_UI']
                            query_UI = episode_data['query_UI']
                            instru_action = episode_data['instru_action']
                            reflect = episode_data['reflect']
                            #summary_prompt = episode_data['summary_prompt']
                            ...
                            if is_successful and task_template not in task_completed:
                                task_completed.append(task_template)
                            task_all.append((task_template, is_successful))
                            if is_successful:
                                s += len(action_prompt)
                            else:
                                f += len(action_prompt)
                            for idx, step_prompt in enumerate(action_prompt):
                                ...
                                ui_elements = episode_data['before_element_list'][idx]
                                ...
                                if (idx == 0 and random.random() > 0.8) or idx != 0:
                                    summary_UI_current = summary_UI[idx]
                                    summary_UI_list, lost_num1 = extract_json_objects(summary_UI_current)
                                    for question_pair in summary_UI_list:
                                        taskA_query = GENERAL_PROMPT.format(ui_elements_description=before_element_list, question=question_pair['Question'])
                                        general_task_data.append({'query':taskA_query, 'response':question_pair['Answer']})
                                    lost_simple_data += lost_num1

                                    query_UI_current = query_UI[idx]
                                    query_UI_list, lost_num2 = extract_json_objects(query_UI_current)
                                    for query_pair in query_UI_list:
                                        #print('>>>>>>>query_pair:', query_pair)
                                        try:
                                            taskA_query = GENERAL_PROMPT.format(ui_elements_description=before_element_list, question=query_pair['Question'])
                                        except:
                                            lost_simple_data += 1
                                            continue

                                        try:
                                            general_task_data.append({'query':taskA_query, 'response':query_pair['Answer']})
                                        except:
                                            lost_simple_data += 1
                                            continue
                                    
                                    lost_simple_data += lost_num2
                                    
                                 
    general_task_data_file = '/general_task_data.json'
    instru_task_data_file = '/instru_task_data.json'
    traj_task_data_file = '/traj_task_data.json'
    traj_dpo_task_data_file = '/traj_dpo_task_data.json'

    with open(general_task_data_file, 'w', encoding='utf-8') as json_file:
        json.dump(general_task_data, json_file, ensure_ascii=False, indent=4)

    with open(instru_task_data_file, 'w', encoding='utf-8') as json_file:
        json.dump(instru_task_data, json_file, ensure_ascii=False, indent=4)

    #print(f'>>test:{instru_task_data[1]["response"]}')
    with open(traj_task_data_file, 'w', encoding='utf-8') as json_file:
        json.dump(traj_task_data, json_file, ensure_ascii=False, indent=4)

    with open(traj_dpo_task_data_file, 'w', encoding='utf-8') as json_file:
        json.dump(traj_dpo_task_data, json_file, ensure_ascii=False, indent=4)

    print('general_task_data:', len(general_task_data))
    print('instru_task_data:', len(instru_task_data))
    print('traj_task_data:', len(traj_task_data))
    print('traj_dpo_task_data:', len(traj_dpo_task_data))
    print('lost_simple_data:', lost_simple_data)
    print('nan_num:', nan_num)
    print('score_dict_keys:', score_dict_keys)
    print('score_extract:', score_extract)
    print('scores_fail:', scores_None)

    return task_completed, task_all


folder_path = ''
task_completed, task_all = process_pkl_gz_files(folder_path)
print('\n')
print('num_task_success:', len(task_completed))
print('num_task_all:', len(task_all))

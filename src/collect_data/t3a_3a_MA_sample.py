...
PLAUSIBLE_ACTION_SELECTION_PROMPT_TEMPLATE = (
    ...
    ' can only output incorrect answer (must be absolutely wrong action and plausible reason, if you have no choice, you can make up a wrong action) from the above list in the correct JSON format,'
    ' following the reason why you do that. Your answer should look like:\n'
    'Reason: ...\nAction: {{"action_type":...}}\n\n'
    'Your answer (Remember you are playing a clumsy agent, your answer must be wrong action):\n'
    )
...
class T3A(base_agent.EnvironmentInteractingAgent):
  ...
  def __init__(
    ...
    self.qwen = infer_qwen.Gpt4Wrapper('qwen')
    self.history = []
  ...
  async def step(self, goal: str) -> base_agent.AgentInteractionResult:
    step_data = {
        ...
        'decision_action_output_list': None,
        'action_list': None,
        'reason_list': None,
        'decision_action_reward_list': None,
        'action_adapt': None,
        ...
        'reflect': None,
        'MA_sample': None,
        'agent_actions_rewards': None,
        'agent_plauactions_rewards': None
    }
    ...
    reward_num = 1
    plauaction_output_reward = []
    for i in range(reward_num):
      print('\nAgent plaustep sample----')
      answers = await self.graph_agent.evaluate([plausible_action_prompt, plausible_action_prompt, plausible_action_prompt, plausible_action_prompt]) 
      ...
      print('\nEvaluating Agent plaustep Scores----')
      ...
      action_reward_dict = {
        ...
        'action_reward': scores
      }
      plauaction_output_reward.append(action_reward_dict)
      

    step_data['agent_plauactions_rewards'] = plauaction_output_reward

    reward_num = 3
    action_output_reward = []
    scores = ''
    for i in range(reward_num):
      score_text = scores if scores else "None"
      action_prompt1 = action_prompt + '\n' + 'Last step\'s scores from expert evaluator:\n' + score_text
      print('\nAgent normal step sample----')
      answers = await self.graph_agent.evaluate([action_prompt1, action_prompt1, action_prompt1, action_prompt1]) 
      action_output_list = [answer["output"] for answer in answers]
      raw_response_list = [answer["raw_response"] for answer in answers]
      ...
      action_output_reward.append(action_reward_dict)
      
    step_data['agent_actions_rewards'] = action_output_reward
    ...
    max_frequency, converted_action_adapt_with_idx = get_most_common_action_type_with_indices(converted_action_list)
    converted_action_adapt = converted_action_adapt_with_idx[1]
    converted_action_idx = converted_action_adapt_with_idx[0]
    #converted_action_adapt = converted_action_list[0]
    converted_action_reason = reason_list[converted_action_idx]
    step_data['action_adapt'] = converted_action_adapt.action_type
    step_data['action_adapt_frequency'] = max_frequency
    print('------action_adapt:{}, {}'.format(converted_action_adapt, max_frequency) )
    ...
    step_data['summary'] = (
        f'Action selected: {converted_action_adapt}. {summary}.'

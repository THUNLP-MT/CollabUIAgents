...
#from android_world.agents import t3a_3a
#from android_world.agents import t3a_3a_step_reward
from android_world.agents import t3a_3a_collect_data
from android_world.agents import t3a_3a_MA_sample
from android_world.agents import t3a_3a_SA_sample
...
_DEVICE_CONSOLE_PORT = flags.DEFINE_integer(
    ...
    ' first connected device is port 5554, the second is 5554, and'
    ' so on.',
)
...
_TASK_RANDOM_SEED = flags.DEFINE_integer(
    'task_random_seed', 50, 'Random seed for task randomness.'
)
...
_OUTPUT_PATH = flags.DEFINE_string(
    'output_path', 
    os.path.expanduser('android_world'),
    'The path to save results to if not resuming from a checkpoint is not'
    ' provided.',
)

# Agent specific.
_AGENT_NAME = flags.DEFINE_string('agent_name', 'agent_group', help='Agent name.')
...
def _get_agent(
  ...
  # GPT.
  elif _AGENT_NAME.value == 't3a_qwen2':
    #print('---chose: infer_qwen')
    agent = t3a.T3A(env, infer_qwen.Gpt4Wrapper('qwen'))
  ...
  # agent_group
  elif _AGENT_NAME.value == 'agent_group':
    agent_list = [('COT_LLM_qwen', 0.2), ('COT_LLM_qwen', 0.5), ('COT_LLM_qwen', 0.6), ('COT_LLM_qwen', 0.8)]
    connection_pair = [(0,1), (0,2), (2,1), (1,3), (2,3)]
    
    graph_agent = SimpleGraph(domain="android_world", model_name='gpt-4o')
    #graph_agent = SimpleGraph(domain="android_world", model_name='qwen')
    graph_agent.graph_organize(agent_list, connection_pair)
    agent = t3a_3a_SA_sample.T3A(env, graph_agent)
